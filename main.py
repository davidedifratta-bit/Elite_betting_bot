...
if data["success"] and len(data["data"]) > 0:
    ...
    for m in data["data"]:
        ...
        if over25_odds > 1.90:
            ...
            print("MATCH:", m.get("home_name"), "vs", m.get("away_name"))
            continue
        if int(m.get("o25_potential", 0)) < 40:
            print("STOP O25")
            continue
        ...
        print("MATCHES COMPLETED:", m.get("matches_completed_minimum"))
        ...
        score = (
            ...
            + int(float(m.get("team_a_xg_prematch", 0)) * 25)
            + int(float(m.get("team_b_xg_prematch", 0)) * 25)
            + int(m.get("corners_potential", 0))
        )
        print(
            "SCORE CALCOLATO:",
            m.get("home_name"),
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "signal":
        ...
        league_name = match.get("competition_name", "Unknown League")
